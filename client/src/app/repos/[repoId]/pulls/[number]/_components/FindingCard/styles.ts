...
export const s = {
  card: (focused: boolean, sevColor: string, muted: boolean): CSSProperties => ({
    borderRadius: 8,
    // Fully per-side longhand — React treats `borderColor`/`borderWidth` as
    // shorthand for the 4 side properties and warns ("conflicting property")
    // if a longhand like `borderLeftColor` is also set alongside them.
    borderStyle: "solid",
    borderTopWidth: 1,
    borderRightWidth: 1,
    borderBottomWidth: 1,
